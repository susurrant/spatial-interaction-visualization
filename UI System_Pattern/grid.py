...
class Grid(object):
    ...
    def calcOutFlow_outside(self, scale):
        if 0 in self.toGrid:
            for flow_co in self.toGrid[0]:
                td, ta = calcInteraction(flow_co)
                dx = td*np.cos(ta)*scale
                dy = -1*td*np.sin(ta)*scale
                self.toOutFlowCo.append((self.cenx+dx, self.ceny+dy))
